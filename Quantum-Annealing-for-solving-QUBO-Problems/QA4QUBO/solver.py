...
# estrae dal DWaveSampler il sottografo attivo della QPU 
...
def get_active(sampler, n):
    ...
    for node_1, node_2 in sampler.edgelist: #sampler.edgelist: le connessioni tra qubits
    ...
    try:
        if (not sim):
            ...
            if topology == 'chimera':
                ...
                if (n > 2048):
                    n = int(input(now() + " [" + colors.WARNING + colors.BOLD + "WARNING" + colors.ENDC + f"] {n} inserted value is bigger than max topology size (2048), please insert a valid n or press any key to exit: "))
