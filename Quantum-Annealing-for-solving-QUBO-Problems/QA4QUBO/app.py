...
def app1(TIMES, k, _Q, n, capacity, items):
    ...
    for i in range(I_max.__len__()):
        ...
        for _ in range(TIMES):
            ...
            cluster_max           = max(cluster_sizes.items(), key=lambda kv: kv[1])[0]
            cluster_max_indices   = [idx for idx, label in enumerate(labels) if label == cluster_max]
            cluster_max_solutions = [Z[i] for i in  cluster_max_indices]     
            ...
            string += log_write("cluster sizes      ", cluster_string)
            string += log_write("medoid max cluster ", cluster_max_medoid)
            string += log_write("fQ                 ", medoid_fQ)
