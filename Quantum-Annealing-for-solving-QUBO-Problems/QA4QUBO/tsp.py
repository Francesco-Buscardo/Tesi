...
# Sampler = classe che prende come input un problema e return uno o più sample
# EmbeddingComposite() -> crea un oggetto che mappa un problema sulla topologia del QPU
#
# DWaveSampler() crea un oggetto sampler D-Wave
# EmbeddingComposite() prende quell’oggetto e crea un altro oggetto, 
# cioè un sampler “wrappato” con embedding automatico
# senza EmbeddingComposite() il sampler da DWaveSampler()
# sarebbe già compatibile con la topologia della QPU
#
# DWaveSampler() costruisce un oggetto sampler che invia il problema
# a un solver QPU remoto di D-Wave
def solve_tsp(qubo_dict, k): 
    ...
    # response = annealer(qubo_dict, dimod.ExactSolver(), k)

    return np.array(response)

# LeapHybridSampler() costruisce un oggetto sampler che invia il problema
# a un solver ibrido remoto di D-Wave
def hybrid_tsp(qubo_dict):   
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # response = hybrid(qubo_dict, LeapHybridSampler())      
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    response = hybrid(qubo_dict, neal.SimulatedAnnealingSampler())
    # response = hybrid(qubo_dict, dimod.ExactSolver())
...
def write_TSP_csv(df, dictionary):
    ...
    df.loc[dictionary['type'], 'Solution'] = dictionary['sol']
    df.loc[dictionary['type'], 'Cost'] = dictionary['cost']
    df.loc[dictionary['type'], 'Fixed solution'] = dictionary['fixsol']
    df.loc[dictionary['type'], 'Fixed cost'] = dictionary['fixcost']
    df.loc[dictionary['type'], 'Response time'] = dictionary['rtime']
    df.loc[dictionary['type'], 'Total time'] = dictionary['ttime']
    df.loc[dictionary['type'], 'Response'] = dictionary['response']
...
def tsp(n, DIR, DATA, df, bruteforce=True, DWave=True, Hybrid=True):
    ...
    if DWave:
        ...
        QA['cost'] = round(float(calculate_cost(tsp_matrix, QA['sol'])), 2)

        QA['fixsol'] = list(fix_solution(QA['response'], True))
        QA['fixcost'] = round(float(calculate_cost(tsp_matrix, QA['fixsol'])), 2)
    ...
    if Hybrid:
        ...
        HY['rtime'] = timedelta(seconds = int(time.time() - start_HY)) if int(time.time() - start_HY) > 0 else time.time() - start_HY

        HY['sol'] = binary_state_to_points_order(HY['response'])
        HY['cost'] = round(float(calculate_cost(tsp_matrix,HY['sol'])), 2)

        HY['fixsol'] = list(fix_solution(HY['response'], True))
        HY['fixcost'] = round(float(calculate_cost(tsp_matrix,HY['fixsol'])), 2)
