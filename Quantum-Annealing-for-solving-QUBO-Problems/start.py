...
def main(nn):    
    ...
    # pr = input(colors.OKCYAN + "Which problem would you like to run? (NPP, QAP, TSP, KSP, MKSP)  " + colors.ENDC)
    pr = "KSP"
    ...
    if QAP:
        ...
        nn, C_d, items = ksp.build_mknapsack("QA4QUBO/ksp/mksp_1.txt")

        _DIR = generate_file_ksp(nn, C_d[0])
        log_DIR = _DIR.replace("KSP","KSP_LOG") + ".csv"

        _Q = ksp.generate_QUBO_mksp(nn, C_d, items)
    ...
    if NPP:
        ...
        ksp_bf_weights = sum(items[j][0] for j in range(nn) if choosen[j] == 1)

        if ksp_bf_profit == ksp_dp_profit and ksp_bf_weights == ksp_dp_weight:
            string += colors.BOLD + colors.HEADER + "\nKnapsack Solution" + colors.ENDC + "\n"
            string += log_write("Best profit", ksp_bf_profit)
            string += log_write("Weight", ksp_bf_weights)
            ...
            for i in range(TIMES):
                ...
                w_gaps.append(ksp_bf_weights - total_weights[i])
                gaps.append(percentage_gap(ksp_bf_profit, total_profits[i]))
